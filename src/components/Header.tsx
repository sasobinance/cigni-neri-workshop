...
const TABS = [
  { id: 'presentazione', label: 'Presenta' },
    ...
    <header style={{ background: '#FFFFFF', position: 'sticky', top: 0, zIndex: 100, borderBottom: '1px solid #E4E4E0' }}>
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '0 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, height: 56 }}>
          <img src="/logo.png" alt="I Cigni Neri" style={{ width: 32, height: 32, objectFit: 'contain', flexShrink: 0 }} />
          <div style={{ flexShrink: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: '#2A2A2A', lineHeight: 1.2 }}>I Cigni Neri</div>
            <div style={{ fontSize: 9, color: '#8A8A85' }}>BSC Workshop</div>
          </div>
          <nav style={{ display: 'flex', gap: 3, flexWrap: 'wrap', flex: 1, justifyContent: 'flex-end' }}>
            {TABS.map(t => (
              <button key={t.id} onClick={() => onChange(t.id)} style={{
                padding: '5px 9px', borderRadius: 999, fontSize: 10, fontWeight: active===t.id ? 700 : 500,
                background: active===t.id ? (t.id==='presentazione' ? '#3D3D3D' : '#4A7C59') : 'transparent',
